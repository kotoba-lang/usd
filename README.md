...
`pxr` Python bindings (`pip install usd-core`), shelled out to from JVM-only tests. Set
`USD_ORACLE_PYTHON` to point at a specific interpreter, or install `usd-core` into whatever
